...
- **Spec re-analysis remediation (2026-09-19, 005-agentic-loop `/speckit-analyze`)** — 33 of 39 active FRs verified implemented; the harness built to close SC-004 immediately found a real defect:
  ...
  - **Recorded, not implemented** — T124 FR-022: NFC is missing from the canonical `zen-repo::normalize_alias` used by BOTH clustering (FR-016) and graph verification (FR-015); it exists only in a local `notion/service.rs` fn and inline in `tier4.rs`. T125 FR-040(e): `render_skill_md` emits no Gotchas section (repo-wide grep = 0). T126 FR-011: `last_completed_stage` is a doc-comment-only claim (no column in migrations 001-008; `ProcessingJob` unused; dedup never reads `notes_meta`); recovery is cycle-level only. T127 FR-018: audit is cycle-level (`loop.cycle.completed`/`loop.host.ingested`) — no per-mutation records, and reversibility is txn/CAS rollback + git history only. T128 FR-020: M5 `wiki/virtues/*` is seed-only (virtue logs route to `memories/virtue_logs/`) and there is no M3→M4 belief *creation* path — the latter is a **spec tension** with FR-033 ("Belief creation exclusively via FR-025"), so it needs a spec decision rather than code
