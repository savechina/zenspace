use clap::Subcommand;
use colored::Colorize;
use std::fs;
use tracing::debug;

use zen_core::errors::ZenError;
use zen_core::paths::ZenPaths;

// ---------------------------------------------------------------------------
// Workspace subcommands (FR-003, FR-004, FR-005)
// ---------------------------------------------------------------------------

#[derive(Subcommand)]
pub enum WorkspaceCommands {
    /// Initialize a workspace (.zen directory structure)
    Init {
        /// Target directory (defaults to current directory)
        #[arg(short, long)]
        path: Option<String>,
    },
    /// Show workspace status (directories, config)
    Status,
    /// Cleanup workspace artifacts (cache, temp files)
    Cleanup {
        /// Dry-run: show what would be cleaned
        #[arg(short, long, action)]
        dry_run: bool,
    },
}

pub fn execute_command(operation: &WorkspaceCommands) -> Result<(), ZenError> {
    match operation {
        WorkspaceCommands::Init { path } => {
            let target = path.as_deref().unwrap_or(".");
            let zen_dir = format!("{}/.zen", target);
            debug!("initializing workspace at: {}", zen_dir);

            // Create .zen/ directory structure — project-level config only.
            // Vault, inbox, wiki, skills, sessions, memory, finance are all
            // global (under ZEN_HOME) and managed by ensure_runtime_dirs().
            if fs::metadata(&zen_dir).is_ok() {
                println!(
                    "{} Workspace already initialized at {}",
                    "ℹ".bright_blue().bold(),
                    zen_dir.cyan().bold()
                );
            } else {
                fs::create_dir_all(&zen_dir)?;

                let output_dir = format!("{}/output", zen_dir);
                fs::create_dir_all(&output_dir)?;
                println!("  {} {}", "✓".green().bold(), output_dir.dimmed());

                println!(
                    "\n{} Workspace initialized at {}",
                    "🏗".bright_yellow().bold(),
                    zen_dir.cyan().bold()
                );
            }

            if let Ok(paths) = ZenPaths::detect() {
                match paths.ensure_identity_files() {
                    Ok(()) => {
                        let identity = paths.identity();
                        for file in &["SOUL.md", "MEMORY.md", "AGENTS.md"] {
                            let path = identity.join(file);
                            if path.exists() {
                                println!(
                                    "  {} {}",
                                    "✓".green().bold(),
                                    path.display().to_string().dimmed()
                                );
                            }
                        }
                    }
                    Err(e) => {
                        println!(
                            "  {} Failed to init identity files: {}",
                            "⚠".yellow().bold(),
                            e
                        );
                    }
                }
                let _ = paths.ensure_runtime_dirs();
            }

            // FR-040: sweep for stale tempfile artifacts (.bak/.tmp) left by
            // crashed operations. Best-effort; runs after workspace open/init.
            if let Ok(paths) = ZenPaths::detect()
                && let Some(root) = paths.workspace_root()
            {
                tracing::info!(
                    workspace = %root.display(),
                    "boot_time_sweep: sweeping workspace for stale tempfiles"
                );
                zen_core::tempfile_lifecycle::boot_time_sweep(root);
            }

            Ok(())
        }
        WorkspaceCommands::Status => {
            debug!("showing workspace status");
            let paths = ZenPaths::detect().ok();

            println!("{}", "📂 Workspace Status".bold());

            match &paths {
                Some(p) => {
                    println!(
                        "  Global root: {}",
                        p.global_root().display().to_string().cyan()
                    );
                    match p.workspace_root() {
                        Some(w) => println!("  Workspace root: {}", w.display().to_string().cyan()),
                        None => println!(
                            "  Workspace root: {}",
                            "(not found — no .zen/ in parent dirs)".yellow()
                        ),
                    }
                    println!(
                        "  Config file: {}",
                        p.config_file().display().to_string().dimmed()
                    );
                }
                None => {
                    println!("  Home directory: {}", "not found".red().bold());
                }
            }

            Ok(())
        }
        WorkspaceCommands::Cleanup { dry_run } => {
            debug!("cleaning workspace (dry_run={})", dry_run);
            println!(
                "{} Workspace cleanup{}",
                "🧹".bright_red().bold(),
                if *dry_run {
                    " (dry run)".yellow().to_string()
                } else {
                    String::new()
                }
            );
            println!(
                "{}",
                "  (Service-layer workspace cleanup deferred to zen-service)".dimmed()
            );
            Ok(())
        }
    }
}
